<div class="row">
    <div class="col-lg-4 mb-3">
        <a wire:click="create" href="javascript:void(0)" class="btn btn-success w-50">Create new Account</a>
    </div>
</div>
<div class="row">
    <div class="col-lg-8 mb-3">
        {{-- wire:model="filterUser.query" --}}
        <input type="search" class="form-control w-100"
        placeholder="Search data..." required>
    </div>
    <div class="col-lg-4 mb-3">
        {{-- wire:model="filterUser.role" --}}
        <select class="form-select w-100" >
            <option value="">Semua Tipe Akun</option>
            <option value="ADMIN">Admin</option>
            <option value="STAFF">Staff</option>
        </select>
    </div>
    @if($this->filterUser)
        <div class="col-lg-12 mb-3">
            <button type="button" class="btn btn-primary w-100" wire:click="resetFilterUser"><i class="bx bx-refresh"></i> Reset Filter</button>
        </div>
    @endif
</div>
<div class="overflow-auto table-responsive">
    <table class="table table-bordered table-hover table-striped table-vcenter" wire:loading.class="table-mode-loading">
        <thead>
        <tr>
            <th class="text-center">No.</th>
            <th>Fullname</th>
            <th class="text-center">Email</th>
            <th class="text-center">Role</th>
            <th class="text-center">Biodata</th>
            <th class="text-center">Aksi</th>
        </tr>
        </thead>
        <tbody>
        @if($users->count() == 0)
            <tr>
                <td class="text-center" colspan="5">Data Tidak Ditemukan :(</td>
            </tr>
        @endif
        @foreach($users as $key => $user)
            <tr>
                <td class="text-center fw-semibold">
                    <span>{{ $key + 1 }}</span>
                </td>
                <td>
                    <span>{{ $user->name }}</span>
                </td>
                <td class="text-center">
                    <span>{{ $user->email }}</span>
                </td>
                <td class="text-center">
                    <span class="badge bg-success text-white">{{ $user->getRoleNames()->first() }}</span>
                </td>
                <td class="text-center">
                    <button type="button" wire:click="check_biodata({{ $user->id }})" wire:loading.attr="disabled"
                                class="btn btn-sm btn-success" data-bs-toggle="tooltip" title="Check Biodata">
                        <i class="link-icon" data-feather="user"></i>
                    </button>
                </td>
                <td class="text-center">
                    <div class="btn-group">
                        @if ( $user->getRoleNames()->first() == "STAFF" )
                            <button type="button" wire:click="permission({{ $user->id }})" wire:loading.attr="disabled"
                            class="btn btn-sm btn-warning" data-bs-toggle="tooltip" title="Tambah Permission">
                                <i class="link-icon text-white" data-feather="lock"></i>
                            </button>
                        @endif
                        <button type="button" wire:click="edit({{ $user->id }})" wire:loading.attr="disabled"
                        class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="Sunting Akun">
                            <i class="link-icon" data-feather="edit-2"></i>
                        </button>
                        <button type="button" wire:click="delete({{ $user->id }})"
                        class="btn btn-sm btn-danger" data-bs-toggle="tooltip" title="Hapus Akun">
                                <i class="link-icon" data-feather="trash-2"></i>
                        </button>
                    </div>
                </td>
            </tr>
        @endforeach
        </tbody>
    </table>
    {{-- {{ $users->links('components.pagination') }} --}}
</div>
