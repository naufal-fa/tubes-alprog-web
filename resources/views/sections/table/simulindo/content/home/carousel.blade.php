<div class="row">
    <div class="col-lg-4 mb-3">
        <a wire:click="create" href="javascript:void(0)" class="btn btn-success w-50">Add new Carousel</a>
    </div>
</div>
<div class="row">
        ...
        @foreach($carousels as $key => $carousel)
            <tr>
                ...
                <td class="text-center">
                    <img class="rounded w-auto" src="{{ asset('assets/images/content/home/carousel') .'/'. $carousel->image }}" alt="..." wire:click="showImageTab({{ $carousel->id }})">
                </td>
                <td>
                    <span>{{ $carousel->description }}</span>
                </td>
                <td>
                    <span>{{ $carousel->link }}</span>
                </td>
                <td class="text-center">
                    <div class="btn-group">
                        <button type="button" wire:click="edit({{ $carousel->id }})" wire:loading.attr="disabled"
                        class="btn btn-sm btn-primary" data-bs-toggle="tooltip" title="Sunting Akun">
                            <i class="link-icon" data-feather="edit-2"></i>
                        </button>
                        <button type="button" wire:click="delete({{ $carousel->id }})"
